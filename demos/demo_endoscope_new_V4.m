...
min_pixel = 20;      % minimum number of nonzero pixels for each neuron
...
%% check spatial and temporal components by playing movies
 save_avi = true;
 avi_name = 'CaMKII120_Imaging_CNMF_E_Spatial-Temporal Components.avi';
 neuron.Cn = Cn;
 neuron.runMovie(Ysignal, [0, 50], save_avi, avi_name);  %-- (RU) changed Ysignal to Yac
...
saveDir = uigetdir(dir_nm);
saveDir = [saveDir filesep];

% Save footprints as TIFFs
footprints = full(neuron.A);
Ain16 = im2uint16(mat2gray(reshape(footprints, d1, d2, [])));
frameNum = size(neuron.A,2);
if ~exist([saveDir 'footprints'], 'dir')
    mkdir([saveDir 'footprints'])
end

for ii = 1:frameNum
    fullpath = sprintf('%sfootprints/mask%d.tif', saveDir, ii);
    imwrite(Ain16(:,:,ii), fullpath)
end

% Save data into text files
csvwrite([saveDir 'footprints.txt'], footprints)

traces = neuron.C;
csvwrite([saveDir 'traces.txt'], traces)

spikes = neuron.S;
csvwrite([saveDir 'spikes.txt'], spikes)

% Save neuron object
data = neuron;
save([saveDir 'data.mat'], 'neuron');